...
def CreateMesh(Start_lon, End_lon, Step_lon, Start_lat, End_lat, Step_lat):
    ...
    #get the dimension length of the mesh
    range_lat = (int) ((End_lat - Start_lat)/Step_lat)
    range_lon = (int) ((End_lon - Start_lon)/Step_lon)
    
    
    Center_lon= [] # to hold the longitude of each cell's center in the grid
    Center_lat= [] # to hold the latitude of each cell's center in the grid
    Grid_FRP = []  # to be ready to hold the accumualted FRP of each cell in the grid
...
def CreateGridFRP(dt, files):
    ...
    for f in files:       
        ...
        # these two variables are to check the correctness of the calculation
        MismatchedFRP = 0.0
        tmp_total_FRP = 0.0
        ...
        # place each fire pixel from the  List Product file into the correct 5-deg grid cell
        ...
        for i in range(len(FRP)):
            ...
            if (Start_lon <= Lon[i] <= End_lon) and (Start_lat <= Lat[i] <= End_lon):       
                ...
                #pixel i is within our interested area
                ...
                #pixel i is outside of our interested area, record the mismatch value
                MismatchedFRP += FRP[i]         
                
        # To show if the FRP is balanced
        balance = sum(FRP) - tmp_total_FRP - MismatchedFRP
        if abs( balance ) > ZERO :
            print( "FRP error=%lf" % balance)
        #pdb.set_trace()          
